...
int[,] TurnArray(int[,] inputArray)
{
    int[,] resultArray = new int[inputArray.GetLength(1), inputArray.GetLength(0)];
    
    for (int i = 0; i < inputArray.GetLength(0); i++)
    {
        for (int j = 0; j < inputArray.GetLength(1); j++) resultArray[j, i] = inputArray[i, j];
    }
    return resultArray;
}
